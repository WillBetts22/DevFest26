import { isJSONSchema, SUPPORTED_STRING_FORMATS } from "./jsonschema.mjs";
/**
...
export function transformJSONSchema(schema) {
    if (schema.type !== 'object' && !schema.anyOf && !schema.oneOf && !schema.allOf) {
        throw new Error(`Root schema must have type: 'object' or use anyOf/oneOf/allOf, but got type: ${schema.type ? `'${schema.type}'` : 'undefined'}`);
    }
 ...
 */
function ensureStrictSchema(jsonSchema, path, root) {
    ...
    if (isObject(defs)) {
        for (const [defName, defSchema] of Object.entries(defs)) {
            ensureStrictSchema(defSchema, [...path, '$defs', defName], root);
        }
    }
    ...
    if (isObject(definitions)) {
        for (const [defName, defSchema] of Object.entries(definitions)) {
            ensureStrictSchema(defSchema, [...path, 'definitions', defName], root);
        }
    }
        ...
        if (isObject(properties)) {
            ...
            for (const [key, value] of Object.entries(properties)) {
                if (!isNullable(value) && !required.includes(key)) {
                    throw new Error(`Property '${key}' at path '${[...path, 'properties', key].join('/')}' uses optional ` +
    ...
    if (Array.isArray(anyOf)) {
        jsonSchema.anyOf = anyOf.map((variant, i) => ensureStrictSchema(variant, [...path, 'anyOf', String(i)], root));
    }
    // Handle oneOf
    const oneOf = jsonSchema.oneOf;
    if (Array.isArray(oneOf)) {
        // Convert oneOf to anyOf (more lenient)
        jsonSchema.anyOf = oneOf.map((variant, i) => ensureStrictSchema(variant, [...path, 'oneOf', String(i)], root));
    ...
    if (Array.isArray(allOf)) {
        if (allOf.length === 1) {
            ...
            jsonSchema.allOf = allOf.map((entry, i) => ensureStrictSchema(entry, [...path, 'allOf', String(i)], root));
        }
    }
