...
function createLogger(client, verbose, debug) {
    ...
    return {
        ...
        toolExecution: (name, result, error = false) => {
            if (!verbose)
                return;
            const stringified = (0, format_1.jsonify)(result);
        ...
        messagesSnapshot: (messages) => {
            ...
            messages.slice(-6).forEach((msg, idx) => {
                ...
                if (Array.isArray(content)) {
                    ...
                    const str = (0, format_1.jsonify)(content ?? '');
...
class DedalusRunner {
    ...
    async run(params) {
        ...
        const toolHandler = (0, tools_1.createToolHandler)(tools ?? []);
        const logger = createLogger(this.client, this.verbose ?? !!verbose, !!debug);
        logger.toolSchema(toolHandler.listNames());
        const modelSpec = (0, validation_1.normalizeModelSpec)(model);
        const requestKwargs = (0, validation_1.buildRequestKwargs)(apiParams);
        const state = {
            model: modelSpec,
            requestKwargs,
            autoExecuteTools,
        ...
        const conversation = (0, messages_1.buildInitialMessages)({
            instructions,
            input,
            messages,
        });
    ...
    async runTurns(conversation, state) {
        ...
        while (steps < state.maxSteps) {
            ...
            if (state.stream) {
                ...
                for await (const chunk of response) {
                    if (chunk.choices) {
                        ...
                        if (delta) {
                            if (delta.tool_calls) {
                                (0, streaming_1.accumulateToolCalls)(delta.tool_calls, collectedToolCalls);
                            }
                            if (delta.content) {
            ...
            const toolPayloads = toolCalls.map((call) => (0, messages_1.coerceToolCall)(call));
    ...
    async *runStreaming(conversation, state) {
        ...
        while (steps < state.maxSteps) {
            ...
            const toolPayloads = localToolCalls.map((call) => (0, messages_1.coerceToolCall)(call));
            history.push({ role: 'assistant', tool_calls: toolPayloads });
            if (!state.autoExecuteTools)
                break;
            const toolResults = [];
...
exports.DedalusRunner = DedalusRunner;
