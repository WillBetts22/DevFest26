import type { Message } from "./types/messages.mjs";
/** Constructs initial conversation history from input parameters. */
export declare function buildInitialMessages({ instructions, input, messages, }: {
    instructions?: string | undefined;
    input?: string | Message[] | undefined;
    messages?: Message[] | undefined;
}): Message[];
/** Normalizes tool call to standard format with required fields. */
export declare function coerceToolCall(call: any): any;
//# sourceMappingURL=messages.d.mts.map